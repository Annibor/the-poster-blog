...
class PostList(generic.ListView):
    ...
    def get_queryset(self):
        """
        Overrides the default queryset to filter posts by status,
        annotate each post with
        the count of approved comments,
        and order the posts by their creation date in descending order.
        """
        return Post.objects.filter(status=1).annotate(
            approved_comments_count=Count('comments',
                                          filter=Q(comments__approved=True))
        ).order_by('-created_on')

    def get_context_data(self, **kwargs):
        """
        Extends the base implementation to add the
        ist of posts to the context.
        """
...
class PostDetail(DetailView):
    """
    A view that inherits from Django's DetailView for
    ...
    def get_context_data(self, **kwargs):
        """
        Overrides to add comments, comment form, and comment
        count to the context for the post.
        """
        context = super().get_context_data(**kwargs)
        post = context['post']
...
class CommentCreate(View):
    """
    A view handling the creation of a new comment for a specific post.
    This view processes
    the POST request submitted through the comment form.
    """
    def post(self, request, slug):
        """
        Handles POST request. If the comment form is valid,
        ...
        if comment_form.is_valid():
            ...
            # Redirect to the blog main page after successful
            # comment submission.
...
class CommentUpdate(LoginRequiredMixin, View):
    ...
    user is authenticated and authorized to edit the
    ...
    def post(self, request, *args, **kwargs):
        """
        Handles POST requests. It updates a
        comment's body with the new content provided by the user.
        ...
        # Extract the 'body' field from the JSON data,
        ...
        if not body:
            return JsonResponse({'status': 'error', 'message':
                                 'Comment body cannot be empty'}, status=400)
...
class LikePost(LoginRequiredMixin, View):
    """
    Allows a logged-in user to like or unlike a post.
